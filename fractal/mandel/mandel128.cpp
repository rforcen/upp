...
public:
  ...
  Mandelbrotf128(u32 *image, u32 w, u32 h, u32 iters, Complex64 center,
                 Complex64 range)
      : w(w), h(h), iters(iters), center(center), range(range), image(image),
        cr(Complex128(range.real(), range.real())),
        rir((range.imag() - range.real())), scale(0.8 * f128(w) / h) {}
  ...
  void gen_pixel(int index) {
    const Complex128 c0 =
        scale * do_scale(f128(index % w) / w, f128(index / w) / h) - center;
    Complex128 z = c0;
    ...
    image[index] =
        0xff000000 | ((ix == iters) ? 0 : fire_pallete[(ix << 2) % 256]);
  }
...
extern "C" {
void genMandelbrotMTf128(u32 *image, u32 w, u32 h, u32 iters, Complex64 center,
                         Complex64 range) {
