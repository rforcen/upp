...
void test04() {
  ...
  auto n0 = NC<double>::random(200, 300, 20, 20);

  NC d;

  for (int i = 0; i < 3; i++) {
    n0.rand();
    printf("lap det: %ld ms\n", Timer().chrono([&] { d = n0.det(); }));
  }

  d.first(10).print("first 10 d:");
  cout << endl;

  n0.save("n0");
  d.save("d");

  puts("saved files n0, d");
  puts(
      "check python: import numpy as np; n0=np.load('n0'); d=np.load('d'); "
      "np.max((np.linalg.det(n0)-d)/d)");
}
...
void test17(size_t n = 5) {  // filter
  ...
  n1 = NC<double>::random(10000000);  // large # items
  auto nc1 = n1.count([=](double x) { return ((x >= 0.7) && (x <= 0.9)); });
  ...
  auto nc2 = n1.count([=](double x) { return x == 0; });
  puts((nc1 == nc2) ? "ok" : "fail");
...
void test18(int n = 400) {
  ...
  NC m1;
  auto _cmpFunc = [=](double x) { return x > 0.6 && x < 0.8; };  // cmp func

  auto lapMT = Timer().chrono([&] { m1 = n1.match(_cmpFunc); });
  auto lapST = Timer().chrono([&] { m1 = n1.matchST(_cmpFunc); });

  m1.first(10).sort().print("m 0..10:");
  m1.last(10).sort().print("m ..10:");

  printf("lapST:%ld ms, lapMT:%ld ms\n", lapST, lapMT);

  puts((m1.count(_cmpFunc) == m1.size && n1.count(_cmpFunc) == m1.size)
           ? "test ok"
           : "failure");
}
...
void test22() {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    printf("%s, (%20ld,%20ld) ",
           (sa == a.sum()) ? "decompress ok" : "decompression failure!", chsa,
           a.checksum());
    puts((chsa == a.checksum()) ? "checksum ok" : "cheksum failure!");
  }
}
