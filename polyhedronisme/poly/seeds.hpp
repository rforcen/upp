...
class Seeds : public Polyhedron {
 public:
  ...
  static Polyhedron antiprism(int n = 4) {
    float theta = (2. * M_PI) / n,  // pie angle
        h = sqrtf(1. -
                  (4. / ((4. + (2. * cosf(theta / 2.))) - (2. * cosf(theta))))),
          r = sqrtf(1. - (h * h)),
    ...
    for (int i = 0; i < n; i++)
      vertexes.push_back(
          Vertex{r * cosf((i + 0.5) * theta), r * sinf((i + 0.5) * theta), -h});
  ...
  static Polyhedron cupola(int n = 3, float alpha = 0, float height = 0) {
    ...
    float s = 1.0,  // alternative face/height scaling
        rb = s / 2 / sinf(M_PI / 2 / n), rt = s / 2 / sinf(M_PI / n);
    ...
    for (int i = 0; i < n; i++) {
      vertexes[i * 2] =
          Vertex{rb * cosf(M_PI * (2 * i) / n + M_PI / 2 / n + alpha),
                 rb * sinf(M_PI * (2 * i) / n + M_PI / 2 / n + alpha), 0.0};
      vertexes[2 * i + 1] =
          Vertex{rb * cosf(M_PI * (2 * i + 1) / n + M_PI / 2 / n - alpha),
                 rb * sinf(M_PI * (2 * i + 1) / n + M_PI / 2 / n - alpha), 0.0};
      vertexes[2 * n + i] = Vertex{rt * cosf(2 * M_PI * i / n),
                                   rt * sinf(2 * M_PI * i / n), height};
    }
    ...
    for (int i = 0; i < n; i++) {  // n triangular sides and n square sides
      faces.push_back(Face{(2 * i + 1) % (2 * n), (2 * i + 2) % (2 * n),
                           2 * n + (i + 1) % n});
      faces.push_back(
          Face{2 * i, (2 * i + 1) % (2 * n), 2 * n + (i + 1) % n, 2 * n + i});
    }
  ...
  static Polyhedron anticupola(int n = 3, float alpha = 0, float height = 0) {
    ...
    // fill vertexes
    for (int i = 0; i < n; i++) {
      vertexes[2 * i] = Vertex{rb * cosf(M_PI * (2 * i) / n + alpha),
                               rb * sinf(M_PI * (2 * i) / n + alpha), 0.0};
      vertexes[2 * i + 1] =
          Vertex{rb * cosf(M_PI * (2 * i + 1) / n - alpha),
                 rb * sinf(M_PI * (2 * i + 1) / n - alpha), 0.0};
      vertexes[2 * n + i] = Vertex{rt * cosf(2 * M_PI * i / n),
                                   rt * sinf(2 * M_PI * i / n), height};
    }
    // create faces
    ...
    for (int i = 0; i < n; i++) {  // n triangular sides and n square sides
      faces.push_back(
          Face{(2 * i) % (2 * n), (2 * i + 1) % (2 * n), 2 * n + (i) % n});
      faces.push_back(Face{2 * n + (i + 1) % n, (2 * i + 1) % (2 * n),
                           (2 * i + 2) % (2 * n)});
      faces.push_back(
          Face{2 * n + (i + 1) % n, 2 * n + (i) % n, (2 * i + 1) % (2 * n)});
    }
  ...
  static Polyhedron johnson(int j) {
    ...
    if (ffv == johnsons.end())
      return Polyhedron();  // not found -> return empty poly

    auto fv = ffv->second;  // map value {faces, vertexes}
    return {"J" + to_string(j), 0, fv.second, fv.first};
  }

  static Polyhedron polyhedron(char name = 'T', int n = 0, float alpha = 0,
                               float height = 0) {
    switch (name) {
      case 'T':
        return tetrahedron();
